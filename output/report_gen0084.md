...
| vault window | 2023-01-03 .. 2026-09-25 |
| identity | data `bc7309ffbceca3e1` · panel `bb48274e99722e85` · config `dd373ebf180943a7` |
| platform | x86_64linux (vendored siblings) |
| family | seasonal_rule |

## Equity, rolling Sharpe, drawdown

![report_gen0084_equity.png](report_gen0084_equity.png)
...
![report_gen0084_rolling_sharpe.png](report_gen0084_rolling_sharpe.png)

*Rolling 3-year net Sharpe. Flat stretches below zero are what a single headline Sharpe hides.*

![report_gen0084_drawdown.png](report_gen0084_drawdown.png)
...
| vault net Sharpe | +1.528 |
| vault days | 936 |
| Sharpe at 2x costs | +1.141 |
| bootstrap 95% CI of net Sharpe | [+0.939, +1.714] |
| CPCV paths net-positive | 100% of 28 (median path SR +1.17) |
| P(drawdown > 40% in 2 years) | 0.000 |

### Deflated Sharpe and PBO

- **DSR 0.0000 at N = 3036 ledger trials, 11 vault trials** (sr0 threshold 0.1455, T = 6041 days, skew 0.148, kurtosis 6.052). The deflation uses the EMPIRICAL spread of trial Sharpes from the ledger, never a hardcoded count.
- Vault DSR **0.9933** at N = 11 vault trials — the count of times any candidate has been shown the post-2023-01-01 data at all.
- **PBO n/a** (CSCV, 33420b3d8d1b is not one of the 32 genomes in the generation-84 returns matrix, so that cohort's PBO is not evidence about it) — **this candidate is not in the cohort matrix, so PBO is unmeasured and gate G4 fails on that alone.**
...
|---|---|---|---|---|
| G1 | like-for-like | bc7309ffbceca3e1 / bb48274e99722e85 / dd373ebf180943a7 | complete identity | PASS |
| G2 | DSR (pre-vault) | 0.000 | 0.950 | **FAIL** |
| G3 | vault confirmation | 1.528 / 0.993 | 0.000 / 0.900 | PASS |
| G4 | PBO (CSCV) | n/a | 0.200 | **FAIL** |
| G5 | CPCV 28 paths | 1.000 / 1.172 | 0.700 / 0.300 | PASS |
| G6 | bootstrap Sharpe CI | 0.939 | 0.000 | PASS |
| G7 | 2x cost stress | 1.141 / 0.859 | 0.000 / 0.500 | PASS |
...
|---|---:|---:|---|
| 2000-03-01 .. 2002-10-31 | +96.6% | 671 |  |
| 2008-09-01 .. 2009-03-31 | -7.2% | 146 | under the soft floor (-5%) |
...
- Gross cumulative return **+21115.6%**, net **+11183.3%** over 6041 pre-vault days.
- Cost drag **264 bps/year** of equity; mean daily turnover **18.3%** of equity.
- Total frictions paid: **$499,082** on a $15,000 account.
- **Cost share of gross: 11.2%**

## Trial ledger

- **3036 distinct genomes** have been evaluated at some fidelity and are on the ledger. That is the N every deflated Sharpe above is deflated by.
- **12 vault accesses** logged. Every look at post-2023-01-01 data is counted, including the ones that only stored an artifact.
...
| 2 | `eaf88989f23d` | seasonal_rule | +1.312 | 84 | 84 | crossover | `eaf88989f23d` x `eaf88989f23d` |
| 3 | `086f728ca68a` | seasonal_rule | +1.311 | 72 | 72 | crossover | `58f40d721b75` x `21b999a10e40` |
| 4 | `7f05a1154176` | seasonal_rule | +1.311 | 73 | 73 | mutate | `086f728ca68a` |
| 5 | `1aec4f9c8871` | seasonal_rule | +1.307 | 49 | 49 | mutate | `84c31eead7a7` |
| 6 | `84c31eead7a7` | seasonal_rule | +1.299 | 50 | 50 | mutate | `4c930c7c5ac4` |
| 7 | `f92d4442d863` | seasonal_rule | +1.299 | 55 | 55 | mutate | `1e5d16cade27` |
| 8 | `8c4bf0eb0bee` | seasonal_rule | +1.299 | 49 | 49 | mutate | `c78907493761` |
| 9 | `c78907493761` | seasonal_rule | +1.299 | 48 | 48 | mutate | `f96cf3d59188` |
| 10 | `0196a6949756` | seasonal_rule | +1.299 | 68 | 68 | crossover | `e5fdf56e5606` x `26852b69a422` |
